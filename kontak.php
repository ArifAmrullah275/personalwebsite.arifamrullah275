...
<header class="bg-green-700 dark:bg-green-900 text-white fixed top-0 w-full shadow-md z-50">
  <div class="flex items-center justify-between max-w-6xl mx-auto px-6 py-4">
    ...
    <div class="flex items-center space-x-4">
      ...
      <span class="text-2xl font-bold">Pesan | <i>Arif Amrullah 275</i></span>
    </div>
...
<main class="max-w-2xl mx-auto pt-32 p-6">
    <h2 class="text-xl font-semibold mb-4">Silakan tinggalkan pesan Anda:</h2>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $sql = "INSERT INTO tbl_buku_tamu (nama, email, pesan) VALUES ('$nama', '$email', '$pesan')";
        if (mysqli_query($db, $sql)) {
            echo "<div class='bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4'>Pesan berhasil dikirim!</div>";
        } else {
            echo "<div class='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4'>Terjadi kesalahan: " . mysqli_error($db) . "</div>";
        }
    }
    ?>
...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">    
    <form method="POST" class="space-y-4">
        <div>
            ...
            <input type="text" name="nama" required class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-green-400">
        </div>
        <div>
            <label class="block mb-1 font-medium">Email</label>
            <input type="email" name="email" required class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-green-400">
        </div>
        <div>
            <label class="block mb-1 font-medium">Pesan</label>
            <textarea name="pesan" rows="5" required class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-green-400"></textarea>
        </div>
        <button type="submit" class="bg-green-600 text-white px-6 py-2 rounded hover:bg-green-700">Kirim</button>
    </form>
</div>
</main>
